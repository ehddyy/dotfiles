...
alias v='vim'
alias meetings='vim ~/Documents/meetings/meetings.md'
alias BI='cd ~/Documents/projects/'

# CHMOD
chmoding() {
 echo "Chmoding $1 folders"
 find $1 -type d -exec sudo chmod 777 {} \;
 echo "Chmoding $1 files"
 find $1 -type f -exec sudo chmod 777 {} \;
 echo "Chmoded $1"
}

kdpgroup() {
	kubectl get pods -n $2 --no-headers=true | awk '/group-react/$1'| xargs kubectl delete -n $2 pod
}
...
dw() {
 cd ~/dev/dw/
 source ~/Documents/venv/dw/bin/activate
}
alias analytics-client='clickhouse-client -h 51.158.114.192 --port 9000 -u default --password WGwbLFjIGdyXIBLs%' 
alias analytics-local='docker-compose exec click_server clickhouse-client'
...
alias la='ls -A'
alias l='ls -F'
command -v lsd > /dev/null && alias ls='lsd --group-dirs --config-file ~/Documents/lsd.config first' && \
	alias tree='lsd --config-file ~/Documents/lsd.config --tree '
...
export PYTHONPATH=$PYTHONPATH:$HOME/dev/anonymisation/
export PATH=$PATH:$HOME/dev/anonymisation/
...
# JIRA Plugin
export JIRA_URL=https://powens.atlassian.net
export JIRA_PREFIX=
export JIRA_NAME="Antoine Auer"
export JIRA_RAPID_VIEW=194
export JIRA_RAPID_BOARD=true
export JIRA_DEFAULT_ACTION=dashboard
export JIRA_TEMPO_PATH=
...
_source_plugin() {
	local plugin_name="$1"
	for basedir in /usr/share/zsh/plugins /usr/share ${ZSH_CUSTOM:-~/.oh-my-zsh/custom}/plugins ${ZSH_CUSTOM:-~/.oh-my-zsh/plugins} 
	do
...
if ! [[ $(tty) = /dev/tty* ]]
then
	if source ${ZSH_CUSTOM:-~/.oh-my-zsh/custom}/themes/powerlevel10k/powerlevel10k.zsh-theme 2> /dev/null
	then
...
function vpn () {
	case $1 in
		down)
		nmcli connection down "VPN_TROUS"
		;;
	up)
		echo -n "Yubikey TOTP: "
		IFS= read -rs yubikey < /dev/tty
		echo
		nmcli connection modify "VPN_TROUS" +vpn.data user=$yubikey
	 	nmcli connection up "VPN_TROUS" -a
	;;
	*)
		echo "up or down"
		;;
...
source '/home/aauer/.config/nvm/versions/node/v21.6.0/lib/node_modules/@hyperupcall/autoenv/activate.sh'
source '/home/aauer/.config/nvm/versions/node/v21.6.0/lib/node_modules/@hyperupcall/autoenv/activate.sh'
source '/home/aauer/.npm-global/lib/node_modules/@hyperupcall/autoenv/activate.sh'
source '/home/aauer/.npm-global/lib/node_modules/@hyperupcall/autoenv/activate.sh'
