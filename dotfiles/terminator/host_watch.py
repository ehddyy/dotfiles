...
class HostWatch(plugin.Plugin):
        ...
        for terminal in Terminator().terminals:
            if terminal not in self.watches:
                self.watches[terminal] = terminal.get_vte().connect(
                    "contents-changed", self.check_host, terminal
                )
        ...
        if last_line:
            ...
            for prompt_pattern in self.patterns:
                ...
                if match:
                    ...
                    dbg(
                        "prompt match: %s (%s) -> hostname = %s"
                        % (prompt_pattern.pattern, last_line, hostname)
                    )
                    for profile_pattern, profile in self.profile_mappings.items():
                        if hostname == profile:
                            dbg(
                                "profile match: profile %s matches hostname by name"
                                % profile
                            )
                            sel_profile = profile
                            break
                            ...
                            dbg(
                                "profile match: profile %s matches pattern %s on hostname %s"
                                % (profile, profile_pattern.pattern, hostname)
                            )
                            sel_profile = profile
                            break
                        else:
                            dbg(
                                "profile did not match: profile %s, pattern %s, hostname %s"
                                % (profile, profile_pattern.pattern, hostname)
                            )

                    # avoid re-applying profile if no change
                    if (
                        terminal not in self.last_profile
                        or sel_profile != self.last_profile[terminal]
                    ):  # disabled this check, profile wasnt updating for multiple splits. todo: revisit.
                        dbg(
                            "setting profile %s for terminal %s"
                            % (sel_profile, terminal)
                        )
    ...
    def get_last_line(self, terminal):
        ...
        lines = vte.get_text_range(
            start_row, start_col, end_row, end_col, is_interesting_char
        )[0]
        ...
        if lines and lines[0]:
            ...
            if len(lines) <= self.prompt_minlen:
                dbg(
                    "line below prompt min size of "
                    + str(self.prompt_minlen)
                    + " chars : must iterate back"
                )
